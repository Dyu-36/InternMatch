...
</div><Label className="company-file-input cursor-pointer" htmlFor="company-logo"><ImagePlus size={18} aria-hidden="true" /><span>{form.logoUrl ? t("Đổi logo doanh nghiệp") : t("Tải logo doanh nghiệp")}</span><input id="company-logo" type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => handleLogoChange(event.target.files?.[0])} /></Label><p className="ui-hint">{t("PNG, JPG hoặc WEBP, tối đa 5 MB.")}</p></section>
...
<section className="company-form-section"><div className="grid gap-2"><Label htmlFor="company-description">{t("Giới thiệu doanh nghiệp")}</Label><Textarea id="company-description" className="company-textarea" value={form.description} onChange={(event) => update('description', event.target.value)} placeholder={t("Mô tả ngắn về sản phẩm, văn hóa và môi trường làm việc...")} required /></div></section><div className="company-form-actions">{error && <span className="ui-error" role="alert">{t(error)}</span>}{saved ? <span className="company-save-message">{t("Đã lưu thông tin doanh nghiệp.")}</span> : null}<Button type="submit" disabled={busy}>{busy ? <><Loader2 className="animate-spin" aria-hidden="true" /> {t("Đang lưu…")}</> : <><Save size={17} aria-hidden="true" /> {t("Lưu hồ sơ")}</>}</Button></div></form></div></div>;
}
