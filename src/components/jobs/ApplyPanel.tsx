...
export function ApplyPanel({ job }: { job: Job }) {
  ...
  return <section className="ui-card p-6"><h2 className="text-lg font-extrabold">{t("Sẵn sàng ứng tuyển?")}</h2>
    ...
    }}><label className="ui-label" htmlFor="cover-letter">{t("Lời nhắn cho doanh nghiệp")}</label><textarea id="cover-letter" className="ui-textarea" maxLength={5000} value={letter} onChange={e => setLetter(e.target.value)} rows={4} /><p className="ui-hint">{t("Hồ sơ và CV hiện tại sẽ được gửi cùng đơn.")}</p>{error && <p className="ui-error" role="alert">{t(error)}</p>}<Button type="submit" fullWidth loading={busy}>{t("Ứng tuyển ngay")}</Button><Link href="/student/profile" className="ui-button ui-button-secondary ui-button-full">{t("Cập nhật hồ sơ / CV")}</Link></form>}
  </section>;
}
