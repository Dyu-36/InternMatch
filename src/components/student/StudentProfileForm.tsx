...
export default function StudentProfileForm() {
  ...
  const handleUniversityOpenChange = (open: boolean) => {
    setUniversityOpen(open);
    if (!open) {
      // A pending request is aborted by the effect cleanup. Reset the loading
      // state as well, otherwise the popover can reopen with a stale loading
      // state after a selection or an interrupted submit.
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      handleUniversityOpenChange(false);
