...
export default function StudentProfileForm() {
  ...
  const handleUniversitySearch = (value: string) => {
    ...
    setSchoolOptions([]);
    setSchoolError('');
    setSchoolLoading(value.trim().length >= 2);
  };
  ...
  <div className="grid gap-2"><Label htmlFor="student-university">{t("Trường Đại học / Viện đào tạo")}</Label><Popover open={universityOpen} onOpenChange={setUniversityOpen}><PopoverTrigger asChild><Button id="student-university" type="button" variant="outline" role="combobox" aria-expanded={universityOpen} className="h-11 w-full justify-between bg-white font-normal text-slate-950 hover:bg-white focus-visible:bg-white"><span className={cn(!form.university && 'text-slate-500')}>{form.university || t("Tìm và chọn trường")}</span><ChevronsUpDown className="size-4 opacity-50" aria-hidden="true" /></Button></PopoverTrigger><PopoverContent align="start" className="w-[var(--radix-popover-trigger-width)] bg-white p-0 text-slate-950"><Command shouldFilter={false} className="bg-white text-slate-950"><CommandInput className="text-slate-950 placeholder:text-slate-500 focus-visible:border-0 focus-visible:outline-none focus-visible:ring-0" placeholder={t("Gõ tên trường để tìm...")} value={universitySearch} onValueChange={handleUniversitySearch} /><CommandList><CommandEmpty className="text-slate-700">{schoolLoading ? t("Đang tải danh sách trường...") : schoolError || (!universitySearch.trim() ? t("Chưa có dữ liệu trường.") : t("Không tìm thấy trường phù hợp."))}</CommandEmpty>{schoolOptions.length > 0 ? <CommandGroup>{schoolOptions.map((school) => <CommandItem key={school.id} value={`${school.id}-${school.name}`} onSelect={() => { updateField('university', school.name); handleUniversitySearch(''); setUniversityOpen(false); }}><Check className={cn("size-4", form.university === school.name ? "opacity-100" : "opacity-0")} aria-hidden="true" /><span className="truncate">{school.name}</span>{school.code ? <span className="ml-auto text-xs text-muted-foreground">{school.code}</span> : null}</CommandItem>)}</CommandGroup> : null}</CommandList></Command></PopoverContent></Popover></div>
