...
export function ResumeLink({ path, name }: { path?: string; name?: string }) {
  ...
  return <span>{url ? <a href={url} target="_blank" rel="noopener noreferrer" className="company-inline-link" onClick={() => setTimeout(() => setUrl(''), 1000)}>{t("Mở")} {name || 'CV'}</a> : <button type="button" className="company-inline-link" disabled={busy} onClick={async () => {
